...
def templateMatching(template, src, method=cv2.TM_SQDIFF):
    """
...
def multiscalingTemplateMatching(template, src, method=cv2.TM_CCOEFF):
    """
    returns crop image of interest
    @template, the template you are searching for
    @src, the source image you are searching in
...
for img in potato_images:
    roi = templateMatching(template, img)
