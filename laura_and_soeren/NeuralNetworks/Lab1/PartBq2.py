...
def train_network(X, Y, Xt, Yt, lr):
                # Create the model
...
def main():
        ...
        for lr in LEARNING_RATES:
                ...
                for num_fold in range( FOLD ):
                        ...
                        val_err_fold.append( train_network( part_X_train, part_Y_train, part_X_test, part_Y_test, lr) )
        ...
        for plotter in range( NUM_EXP ):
            ...
            plt.title('5 fold 32 batch size alpha:%g'%(LEARNING_RATES[plotter]))
            plt.legend()
            plt.savefig('Figures_B2/a%d_valdiationError.png'%(plotter))


            plt.figure(2*plotter + 1)
            plt.plot( range(EPOCHS), val_err_avg[plotter],  label = 'Average LR %g'%(LEARNING_RATES[plotter]))

            plt.xlabel('Epochs')
            plt.ylabel('Average Mean square error')
            plt.title('Average MSE alpha:%g'%(LEARNING_RATES[plotter]))
            plt.legend()
            plt.savefig('Figures_B2/a%d_averagevaldiationError.png'%(plotter))
        ...
        for lr in LEARNING_RATES:
            ...
            te_err = train_network( trainX, trainY, testX, testY, lr)
            ...
            plt.xlabel('Epochs')
            plt.ylabel('Mean square error')
            plt.title('Test error with alpha: %g'%(lr))
            plt.legend()
            plt.savefig('Figures_B2/b%d_testError.png'%i)
            i += 1
